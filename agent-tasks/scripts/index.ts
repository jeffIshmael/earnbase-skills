...
export interface Subtask {
    prompt: string;
    type: 'MULTIPLE_CHOICE' | 'TEXT_INPUT' | 'FILE_UPLOAD' | 'CHOICE_SELECTION' | 'RATING';
    options?: string[];
    required?: boolean;
}
...
export class EarnbaseSkill {
    ...
    async requestHumanTask(
        ...
        } else if (taskSpecs.paymentTxHash) {
            headers['X-PAYMENT'] = taskSpecs.paymentTxHash;
        }
     ...
     * 
     * @param taskSpecs Configuration for the task (title, description, constraints, subtasks)
     */
    ...
    /**
     * Tool: Check the status of a requested task for progress and completion state.
     * 
     * @param requestId The ID returned from requestHumanTask or generated by the agent
     */
    async queryTaskStatus(requestId: string): Promise<{
        status: 'processing' | 'completed' | 'failed';
        progress?: string;
        message?: string;
    }> {
        const response = await fetch(`${this.apiUrl}/api/agent/status/${requestId}`, {
            headers: {
                'X-Agent-Type': 'ERC8004'
            }
        });
        ...
        if (!response.ok) {
            const errorText = await response.text();
            throw new Error(`Earnbase Status Query Failed (${response.status}): ${errorText}`);
        }
    ...
    /**
     * Tool: Check the results of a requested task.
     * Use this once queryTaskStatus returns 'completed'.
     * 
     * @param requestId The ID returned from requestHumanTask or generated by the agent
     */
    async queryTaskResults(requestId: string): Promise<{
        ...
        const response = await fetch(`${this.apiUrl}/api/agent/results?requestId=${requestId}`);

        if (!response.ok) {
            const errorText = await response.text();
            throw new Error(`Earnbase Results Query Failed (${response.status}): ${errorText}`);
        }
